...
| **3D game board** | Pass, Done, Yes/No, skip-bar, playable chips, view menu, zoom +/-, zoom-reset %, stack item | ✓ | ✓ | ✓ | **hotkeys** `P`/Space (pass), `D` (done), `Y`/`N` (ask), `F4` (skip to next turn — legacy Swing key map F3-F11); view-fab + zoom respond to tap |
